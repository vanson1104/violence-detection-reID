...
from re_id import extract_feature, load_network, sort_img, data_transforms, ft_net, fuse_all_conv_bn
import cv2
import torch
import numpy as np
import time
...
# Config detection 
model2_path = 'weight_yolo/best_custom_02.pt'
threshold_iou = 0.1
threshold_conf = 0.5
imgsz = 640
threshold_nms = 0.4
...
linear_num = 512
batchsize = 64
test_dir = "/media/vanson/AI/Lab/violence_detection/Violence_detection/Violence_Detection_Person_reID/pytorch_PersonReID_infermydata"
name = 'ft_ResNet50'
which_epoch = '300'
...
st.markdown('<b style="font-size: 35px;">Ứng dụng nhận diện đối tượng</b>', unsafe_allow_html=True)
...
if uploaded_file is not None:
    ...
    if len(bbox_violence) == 0:
            st.markdown('<b style="font-size: 20px;">Không có hành vi bạo lực</b>', unsafe_allow_html=True)
    t1 = time.time()
    for bbox in bbox_violence:
            cv2.rectangle(image_array, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (255, 0, 0), 2)
            st.markdown('<b style="font-size: 20px;">Hành vi bạo lực được phát hiện</b>', unsafe_allow_html=True)
            # Hiển thị ảnh
    ...
    for i in id_person:
        ...
        person_image = Image.open(query_path)  # Ví dụ này giả định rằng bạn đã cắt ảnh của đối tượng và lưu vào `query_path`
        ...
        for j in range(10):
            ...
            cols[j+1].image(image)  # Cộng 1 vì cột đầu tiên đã được sử dụng để hiển thị ảnh đối tượng
